...
age_gaps_straight %>%
  mutate(verdict_long = check_couple(man_age, woman_age),
         verdict = case_when(
    str_detect(verdict_long, "NOT") ~ "No",
    !str_detect(verdict_long, "NOT") ~ "Yes")) %>%
  ggplot(aes(man_age, woman_age, color = verdict)) +
  geom_jitter(size = 1, alpha = 0.75) +  # Add points for the scatter plot
